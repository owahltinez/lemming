...
test.describe('Environment Overrides UI', () => {
  ...
  test('adds an environment override, stores it in localStorage, and sends it on run', async ({ page }) => {
    ...
    const localStorageData = await page.evaluate(() => localStorage.getItem('lemming_env_overrides'));
