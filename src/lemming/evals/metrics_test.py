...
class TestSourceFacts(unittest.TestCase):
    ...
    def test_function_source_returns_the_definition_verbatim(self):
        # Graders match this text against an agent's file, so a stray blank
        # line or a missing docstring would make them compare the wrong
        # thing and silently pass anything.
        source = (
            '"""Module."""\n'
            "\n"
            "\n"
...
class TestHiddenTests(MetricsTestCase):
    ...
    def test_refuses_to_overwrite_a_workspace_file(self):
        # Silently clobbering agent-authored code would corrupt every other
        # check in the same grading pass.
