...
# Exit code run_hooks records when a runner never started. Task mode reuses
# it so the harness classifies an unstartable agent the same way in both.
_LAUNCH_FAILED = -1
...
def _write_result(
    ...
    payload = {
        ...
        "launch_failed": _LAUNCH_FAILED in codes,
...
def _run_task(
    ...
    Returns:
        ...
        same record.
    """
    # exec reports only whether the task finished, which cannot tell an
    # agent that never started from one that ran and declined to finish.
    # The exit code carrying that distinction is consumed inside the run
    # loop, so it is observed at the one place every agent process passes.
    # Wrapping the module attribute is safe because this entry point runs
    # one trial per process; concurrency happens across containers.
    codes: list[int] = []
    ...
    try:
        lemming_cli.main(
            args=[
                ...
                "--time-limit",
                str(time_limit),
                # A trial is graded from what the run left behind, and the
                # runner log is the only account of how it got there.
                "--keep",
            ],
            prog_name="lemming",
    ...
    # No agent process at all means the loop never reached one, which is
    # the same infrastructure failure as an agent that could not be spawned.
    return {"task": codes[-1] if codes else _LAUNCH_FAILED}
