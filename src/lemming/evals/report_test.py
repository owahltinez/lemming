...
class TestLoad(ReportTestCase):
    ...
    def test_reads_a_report_written_before_config_blocks(self):
        # Reports from an earlier run are a bare list of trials. Refusing
        # to read them would strand results that cost hours to produce.
...
class TestDegenerateScenarios(ReportTestCase):
    ...
    def test_flags_scenarios_where_both_arms_agree_completely(self):
        # A scenario both arms always pass, or always fail, cannot separate
        # them: it costs trials and contributes nothing to the decision.
