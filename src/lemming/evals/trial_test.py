...
# Where the fake agent records the argv it was launched with.
...
class TestTaskModeTrial(TrialTestCase):
    def setUp(self):
        ...
        # The one-shot writes under LEMMING_HOME; never the real one.
    ...
    def test_runs_the_prompt_and_grades_the_workspace(self):
        # A fake agent that records its prompt, works, and reports back.
    ...
    def test_distinguishes_an_agent_that_never_started(self):
        # An unfinished task cannot tell a missing binary from a refusal.
    ...
    def test_an_agent_that_does_nothing_is_not_an_infra_failure(self):
        ...
        # Doing nothing is the grader's verdict, not an infra error.
        checks = self.scenario.grade(self.workspace)
        self.assertFalse(scenarios.passed(checks), checks)

    def test_missing_hook_options_are_reported(self):
        # Hook options are no longer click-required; hook mode still asks.
        result = click.testing.CliRunner().invoke(
