...
# The visible suite's own assertions, copied in only at grading time. That
# suite is in scope and so fair game for the agent to edit; this copy is
# what pins the behavior the fixture actually promised.
...
def _build_fast_exit(workspace: pathlib.Path) -> None:
    ...
    # The clean ops module is fully annotated so it is clean by the Google
    # style guide the hook enforces: an eval run showed the hook (correctly)
    # adding missing annotations, and a fixture it can improve cannot demand
    # a fast exit.
...
def _build_dead_code(workspace: pathlib.Path) -> None:
    ...
    # The duplicate is cleanly formatted, so automated tools stay silent and
    # only a genuine review catches it.
...
def _build_lint_debt(workspace: pathlib.Path) -> None:
    ...
    # Four findings lemming's ruff configuration reports: an unused import,
    # an unsorted import block, a missing docstring, and an over-long line.
    # `readability check --fix` clears three; the docstring needs the hook
    # to write one.
