...
3. **Task Management:** You may manipulate the task list to add or insert new
   ...
   - **Context Isolation:** Be aware that newly scheduled tasks will start with
     ...
     roadmap, previously recorded outcomes from completed tasks, and the state
     of the file system.
   ...
   - **Complex Handoffs:** If you need to pass extensive context (like detailed
     ...
     downstream task, do not rely on the brief outcome messages alone. Instead,
...
4. **Outcomes:** As you progress through the task, you MUST record relevant
   outcomes or technical findings as soon as you discover them (one at a time).
   Do not wait until the end to record everything. Then, at the end of the task,
   before completing or failing it, you MUST record a final outcome summarizing
   the work. A task is considered incomplete without at least one recorded
   outcome. Be thorough and record as many as appropriate:
...
5. **Success:** When you have completely finished and verified the task, and
   recorded ALL relevant outcomes (at least one is required), run:
...
6. **Failure/Blocker:** If you hit a technical roadblock, cannot fix a bug, or
   are unable to complete the task, after recording ALL relevant outcomes (at
